...
Aplicación FastAPI base de `osiris-menu-be`: configuración por entorno, capa de datos async, versionado `/api/v1`, OpenAPI, envelope de error estructurado, idempotencia por `X-Request-Id`, endpoint `/health`, seguridad base de sesión y baseline de migraciones.
