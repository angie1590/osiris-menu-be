...
Orquestación del workspace `osiris-menu/`: un único `docker-compose.yml` raíz que levanta `postgres`, `redis`, `api` y `web` con healthchecks y red interna Docker, Dockerfiles de backend y frontend, y `.env.example`.
